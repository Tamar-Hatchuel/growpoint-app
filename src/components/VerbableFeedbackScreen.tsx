...
const VerbableFeedbackScreen: React.FC<VerbableFeedbackScreenProps> = ({ 
  ...
  const downloadCSV = () => {
    const csvData = [
      ['Date', 'Question', 'Verbal Comment'],
      ...tableRows.map(row => [row.date, row.question, `"${row.comment.replace(/"/g, '""')}"`])
    ];
                ...
                {tableRows.length > 10 && (
                  <div className="text-center pt-3">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="border-growpoint-accent/30 text-growpoint-dark hover:bg-growpoint-soft text-sm"
                    >
                      {showAll ? (
                        <>
